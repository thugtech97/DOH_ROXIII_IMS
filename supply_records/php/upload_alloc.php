...
function insert_ptr(){
	...
	if(count($iids) == count($qtys)){
		...
		for($i = 0; $i < count($iids); $i++){
			$sql = mysqli_query($conn, "SELECT p.po_number, p.item_name, p.description, p.category, p.quantity, p.unit_cost, p.exp_date, s.supplier FROM tbl_po AS p, ref_supplier AS s WHERE p.supplier_id = s.supplier_id AND p.po_id = '".$iids[$i]."'");
				...
				if((int)$qtys[$i] <= (int)$quan_unit[0]){
					...
					if(!in_array($category, $special_category)){
						...
						for($j = 0; $j < count($serials); $j++){
							$sn = $serials[$j];
							mysqli_query($conn, "UPDATE tbl_serial SET is_issued = 'Y' WHERE inventory_id = '".$iids[$i]."' AND serial_no = '$sn'");
						}
					}
					if($property_no != ""){
						if(!in_array($category, $special_category)){
