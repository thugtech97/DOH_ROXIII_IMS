...
function get_item(){
	...
	$sql = mysqli_query($conn, "SELECT i.item_id, i.item, c.category FROM ref_item AS i, ref_category AS c WHERE i.status = '0' AND c.category_id = i.category_id ORDER BY i.item_id ASC");
	if(mysqli_num_rows($sql) != 0){
		while($row = mysqli_fetch_assoc($sql)){
