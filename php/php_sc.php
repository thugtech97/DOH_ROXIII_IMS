...
function get_rpci(){
	...
	while($row = mysqli_fetch_assoc($sql)){
		...
		$sub_total = 0.00;
		$sql2 = mysqli_query($conn, "SELECT p.po_id, p.end_user, p.po_number, p.item_name, s.supplier, p.date_delivered, p.description, p.quantity, p.unit_cost FROM tbl_po AS p, ref_supplier AS s WHERE p.supplier_id = s.supplier_id AND p.category = '$category' AND (p.status = 'Delivered' OR p.status = '')");
...
function print_wi(){
	...
	while($row = mysqli_fetch_assoc($sql)){
		$category = $row["category"];
		$sub_total = 0.00;
		$sql2 = mysqli_query($conn, "SELECT p.end_user, p.po_number, p.item_name, s.supplier, p.date_delivered, p.description, p.quantity, p.sn_ln, p.exp_date, p.unit_cost FROM tbl_po AS p, ref_supplier AS s WHERE p.supplier_id = s.supplier_id AND p.category = '$category'  AND (p.status = 'Delivered' OR p.status = '')");
		if(mysqli_num_rows($sql2) != 0){
			while($row2 = mysqli_fetch_assoc($sql2)){
				$quantity_unit = explode(" ", $row2["quantity"]);
				$total_amount = (float)$quantity_unit[0] * (float)$row2["unit_cost"];
				...
				if($quantity_unit[0] != "0"){
		            ...
		            <td style=\"width: 204.6px; height: 12px; text-align: center; font-size: 8px; vertical-align: center; border-right-color: rgb(0, 0, 0); border-bottom-color: rgb(0, 0, 0); border-right-width: 1px; border-bottom-width: 1px; border-right-style: solid; border-bottom-style: solid;\"><b>".$row2["item_name"]."</b> - ".$row2["description"]."</td>
